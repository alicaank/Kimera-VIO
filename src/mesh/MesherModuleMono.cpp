...
MesherModuleMono::MesherModuleMono(bool parallel_run, Mesher::UniquePtr mesher)
    : MIMOPipelineModule<MesherInputMono, MesherOutput>("Mesher", parallel_run),
      frontend_payload_queue_("mesher_frontend"),
      backend_payload_queue_("mesher_backend"),
      mesher_(std::move(mesher)) {}

MesherModuleMono::InputUniquePtr MesherModuleMono::getInputPacket() {
  ...
  return std::make_unique<MesherInputMono>(
      timestamp, frontend_payload, backend_payload);
}

MesherModuleMono::OutputUniquePtr MesherModuleMono::spinOnce(
    MesherInputMono::UniquePtr input) {
  CHECK(input);
  return mesher_->spinOnceMono(*input);
}

void MesherModuleMono::shutdownQueues() {
  LOG(INFO) << "Shutting down queues for: " << name_id_;
  frontend_payload_queue_.shutdown();
  backend_payload_queue_.shutdown();
};

bool MesherModuleMono::hasWork() const {
